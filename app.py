...
st.set_page_config(page_title="回路構成可否判定シート",
                   layout="wide",
                   initial_sidebar_state="expanded")

st.markdown("""
<style>
  /* Hide Streamlit's top header (fork/share/etc) */
  header {display: none !important;}
  /* Tighter padding and column gaps */
    ...
    with st.expander("📝 Sign Up"):
        ...
        if st.button("Register", key="btn_register"):
            if not su.strip():
                ...
                st.success(f"✅ Account '{su}' created.")
            else:
                st.error(f"Username '{su}' exists")
...
# ─── PAGE 3: Circuit Config ───
    ...
    # — select PCS spec from saved list —
    pcs_list = load_pcs()
    if not pcs_list:
        st.warning("まず「PCS Settings」でPCSを追加してください。")
        st.stop()
    spec = st.selectbox("Select PCS", list(pcs_list.keys()), key="cfg_pcs")
    pcs  = pcs_list[spec]

    # — select module from saved list —
    mods = load_modules()
    if not mods:
        st.warning("まず「Modules」でモジュールを追加してください。")
        st.stop()
    ...
    st.info(f"直列可能：最小 **{min_s}** 枚 ～ 最大 **{max_s}** 枚", icon="ℹ️")

    # — per‐MPPT series inputs & inline validation —
    any_err    = False
    total_mods = 0
    for i in range(mppt_n):
        ...
        for j in range(3):
            ...
            if s>0:
                if s<min_s or s>max_s:
                    c2.error(f"{s}枚は範囲外です ({min_s}～{max_s})。", icon="🚫"); any_err=True
                if ref_s is None:
                    ref_s = s
                elif s!=ref_s:
                    c2.error("同数で統一してください。", icon="🚫"); any_err=True
                total_mods += s
        # check input current
        ...
        if used>0:
            ...
            if cur>i_mppt:
                ...
                c2.error(f"総電流 {cur:.1f}A が PCS許容 {i_mppt}A を超過。", icon="🚫"); any_err=True

    # — final summary —
    if any_err:
        st.error("⚠️ 構成エラー。上記を修正してください。")
    elif total_mods==0:
        ...
        c1.metric("合計モジュール", f"{total_mods} 枚")
